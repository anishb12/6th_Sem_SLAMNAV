...
public:
    void SetExternalCandidateSubscriber(ros::NodeHandle& nh);
protected:
    void ExternalCandidateCallback(const std_msgs::Int32MultiArray::ConstPtr& msg);
    KeyFrame* GetKeyFrameById(long unsigned int id);

    ros::Subscriber mExternalCandSub;
    std::mutex mMutexExternalCand;
    long unsigned int mnExternalCandCurrentKFId = 0;
    long unsigned int mnExternalCandMatchKFId = 0;
    bool mbHasExternalCand = false;
